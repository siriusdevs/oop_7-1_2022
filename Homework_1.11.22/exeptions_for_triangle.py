...
class TriangleInvalidSides(Exception):
    ...
    def __init__(self, sides: List[float or int]) -> None:
        ...
        """
        super().__init__("Triangle with sides: {0}, {1}, {2} - can't exist. Треугольник со сторонами: {0}, {1}, {2} - не существует.".format(*sides))
...
class InvalidCountSidesOfTriangle(Exception):
    ...
    def __init__(self, sides: List[float or int]) -> None:
        ...
        Args:
            ...
            список - список со сторонами треугольника.
        """
        super().__init__("Invalid count of sides in triangle: {0}. Неправильное количество сторон треугольника: {0}".format(sides))
